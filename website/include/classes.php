...
class Application {
    ...
    public function register($username, $password, $email, $registrationcode, &$errors) {
        ...
        if (sizeof($errors) == 0) {
			...
			$url = "https://3i1hea5u86.execute-api.us-east-1.amazonaws.com/default/registeruser";
			$data = array(
				'userid'=>$userid,
				'username'=>$username,
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('x-api-key: AhPzqmhv6Raz0Kpp5k5rh93apugM82Ce2B6hDJ79' ,'Content-Type: application/json','Content-Length: ' . strlen($data_json)));
...
*/
$url = "https://3i1hea5u86.execute-api.us-east-1.amazonaws.com/default/newSession";
$data = array(
    ...
    public function getSessionUser(&$errors, $suppressLog=FALSE) {
        ...
        if (isset($_COOKIE['sessionid'])) {
          ...
          /*  $dbh = $this->getConnection();
            ...
            $dbh = NULL;
*/
$url = "https://3i1hea5u86.execute-api.us-east-1.amazonaws.com/default/getSessionUser";
$data = array(
...
curl_close($ch);
    ...
    public function login($username, $password, &$errors) {
        ...
        // Only try to query the data into the database if there are no validation errors
        if (sizeof($errors) == 0) {
          $url = "https://3i1hea5u86.execute-api.us-east-1.amazonaws.com/default/Login";
          $data = array(
            'username'=>$username
          );
          $data_json = json_encode($data);
          ...
          if ($response === FALSE) {
            ...
            /*  $otp =rand(100000,999999);
             ...
             $headers = 'From: kl03752@georgiasouthern.edu' . "\r\n";
                'Reply-To: kl03752@georgiasouthern.edu' . "\r\n";

                mail($to, $subject, $message, $headers);
                $url = "https://3i1hea5u86.execute-api.us-east-1.amazonaws.com/default/StoreOTP";
                $data = array(
                  'userid'=>$userid,
                  'otp'=>$otp,
                  'email'=>$email
